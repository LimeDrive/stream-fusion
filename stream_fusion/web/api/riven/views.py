...
async def search_by_tmdb_id(yggflix: YggflixAPI, tmdb_id: int, type: str = "movie") -> List[dict]:
    """
...
async def search_yggflix(
    type: MediaTypes = Query(MediaTypes.movie, description="Type of media (movie or tv)"),
    ...
    try:
        ...
        if imdb_id:
            ...
            if final_tmdb_id:
                search_results = await search_by_tmdb_id(yggflix, int(final_tmdb_id), type)
            else:
        ...
        # Keyword search path
            ...
            search_results = await asyncio.to_thread(yggflix.search, query)
            ...
            try:
                torrent_results.append(
                    RivenResult(
                        ...
                        link=result.get("link"),  # TODO: Implement passkey integration
                        seeders=result.get("seeders"),
                        magnet=result.get("magnet"),  # TODO: Implement passkey integration
                        info_hash=result.get("info_hash", "").lower(),
